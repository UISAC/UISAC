...
- Discussions are anonymous to other students but not to the database: nothing about the author is shown or returned by the API, so readers cannot tell who wrote a post, but an author id is stored so people can edit and delete their own posts and admins can moderate. Anyone with Supabase dashboard access can therefore link posts to people, which matters for a board where students ask about visas. Asking, replying and upvoting all require a signed-in Northwestern account. Repeat-vote dedup is per-browser `localStorage`, not server-side.
